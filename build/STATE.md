...
- [ ] 4.12 — **Liturgical retrieval expansion (narrower retry).** Opened from 4.11 diagnostic: 9/10 positive liturgical cases had 0 liturgical candidates reach the judge. Scope: re-attempt liturgical retrieval expansion — narrower than 4.10A's reverted `top_k*5` overfetch (probably 2× for the liturgical index only, not a blanket pool inflation). Gate on seed register where cheap (leverage 4.11's register-gating intuition — if seed is secular-philosophical, don't overfetch liturgical). 4.11's tightened rubric now correctly rejects wrong-feast leakage that would otherwise come with expansion, so the 4.10A judge-side failure mode is closed. First step before coding: verify the baseline-file hypothesis — `git log` on `build/4_7_replay_results.json` to confirm whether the 4.11 baseline was captured during 4.10A. Gate for ship: `lit_pos ≥ 7/10`, hold 4.11 wins (`lit_neg 5/5`, `spillover 5/5`, `prose_liturgy_spillovers ≤ 0`, `prose_flips ≤ 17`).
