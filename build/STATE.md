...
**Last update:** 2026-04-19 (Phase 7 Wave 4.9 diagnosis returned Verdict A [fixture mismatch]; 4.10 hybrid attempted → 4.10A dual-path overfetch REVERTED after net-regression replay [0/10 lit_pos, 4 judge timeouts, 8 new prose spillovers, wrong-feast accepts]; 4.10B seed tightening shipped alone; 4.11 opened for judge calibration)
**Status:** in_progress — Phase 1 complete; Wave 4A + 4B shipped; 4.7 partial (prose ratified); 4.8 shipped as code+ingest partial; 4.9 diagnosis complete (Verdict A); 4.10A reverted + 4.10B shipped; lit_pos gap still open; 4.11 pending (judge adjacency + timeout tuning).
...
- [ ] 4.11 — **Judge calibration for liturgical adjacency + timeout.** Open from 4.10 miss. Scope: (a) tighten `skills/judge_serendipity/SKILL.md` adjacency rubric — Julian of Norwich ≠ Mechthild/Catherine of Siena; Martin of Tours ≠ Memphis Martyrs; proper-date ≠ name match; (b) raise `JUDGE_TIMEOUT` 120s→180s for calls with liturgical candidates; (c) consider register-gating rule (if all candidates look theological, enable liturgical-frame acceptance; if pool is secular-philosophical, suppress liturgical surface). Retrieval stays at 4.8 parity. Gate: replay must show lit_pos ≥7/10, lit_neg hold 5/5, spillover hold 5/5, prose regression ≤ current baseline.
