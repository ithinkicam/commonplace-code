...
**Phase 7 task 4.11 shipped partial + 4.12 opened:** 2026-04-20 (judge-side fixes only — register gate for secular-philosophical seeds + specific-feast match rule with named 4.10A rejects as worked examples in SKILL.md; DEFAULT_JUDGE_TIMEOUT bumped 120→180s in replay harness; replay vs 4.10B baseline: spillover 4/5→5/5, prose_flips 23→17, prose_liturgy_spillovers 8→0, lit_neg 5/5, lit_pos 0/10 unchanged; shipping as partial win since every other axis is strict improvement and safety fixes shouldn't wait; diagnostic revealed 9/10 lit_pos cases had 0 liturgical candidates reach the judge — retrieval IS the bottleneck after all; 4.12 opened for narrower liturgical retrieval expansion with register-gating from 4.11)
**Phase 7 task 4.12 refuted + 4.13 opened:** 2026-04-20 (liturgical 2× overfetch in `surface.py` shipped green on gates, replay FAILED 3/5: `lit_pos 0/10` unchanged, `prose_flips 17→28`, `prose_liturgy_spillovers 0→5`; `lit_neg 5/5` + `spillover 5/5` held; diagnostic revealed `search()` applies `content_type` as a POST-KNN filter — `_KNN_OVERFETCH_MULTIPLIER=5` fetches top-100 nearest chunks across ALL content types then filters, so named-saint LFF collects (Julian, Martin, Benedict, Compline, Ash Wed, Psalm 23, Morning Prayer, Marian — all indexed and embedded; corpus census 1393 liturgical units, 8/8 expected feasts present) never reach the filtered result set because they aren't in the global top-200; `limit*2` scaling can't rescue them; edit reverted; 4.13 opened for filter-aware KNN in `search.py` — run the KNN against the restricted chunk set rather than post-filtering)
**Last update:** 2026-04-20 (Phase 7 task 4.12 refuted — edit reverted; 4.13 opened for filter-aware KNN in search.py)
**Status:** in_progress — Phase 1 complete; Wave 4A + 4B shipped; 4.7 partial (prose ratified); 4.8 shipped as code+ingest partial; 4.9 diagnosis complete (Verdict A); 4.10A reverted + 4.10B shipped; 4.11 shipped partial (judge adjacency + register gate + replay timeout 180s); 4.12 refuted (hypothesis narrower liturgical overfetch disproved by diagnostic — post-KNN filter is the real bottleneck); lit_pos gap still open; 4.13 pending (filter-aware KNN in `search.py`).
...
- [x] 4.9 — **Diagnosis: Verdict A (fixture mismatch).** Pass 1 read-only probe confirmed retrieval works as designed — unfiltered ANN over 768-dim nomic, top-10. `lit_pos_01`→St. Mary collect L2 dist 17.9 (top-10 cutoff 14.9); `lit_pos_10`→Psalm 23 dist 15.25; neither in top-300. `lit_pos_02` *did* return 2 liturgical hits (5773 Confession, 6132 Litany of Penitence) — judge rejected them (separate calibration issue). H2 (hidden filter) + H3 (regression) both falsified. Artifacts: `build/4_9_knn_probe.py` + `.out`. User chose hybrid → 4.10.
- [~] 4.10 — **Hybrid attempted; 4.10A reverted, 4.10B shipped.** Pass 2 replay verdict: net regression. 4.10A (surface.py dual-path `limit*5` overfetch) produced 0/10 lit_pos (no gain over 4.8), 4 judge timeouts at 120s ceiling from oversized pools, 2 wrong-feast judge accepts (Mechthild≠Julian, Memphis Martyrs≠Martin), **8 new prose-register liturgical spillovers across 6 prose seeds** (Labor Day collect, Vocation in Daily Work, Collect for Fridays, etc.) and lit_neg_08 spillover trip (political mercy→Magnificat). Retrieval was not the bottleneck; the Haiku judge collapsed under ~50-candidate pools. User chose option 3: revert 4.10A, keep 4.10B, open 4.11 for judge tuning. Revert restored 1697/1697 baseline; `commonplace_server/surface.py` + `tests/test_surface.py` byte-identical to pre-4.10A HEAD. 4.10B fixture changes (`tests/fixtures/liturgical_surfacing.json` — 10 lit_pos seeds rewritten with composed-embed vocabulary; negatives/spillovers untouched) shipped alone.
- [~] 4.11 — **Judge calibration shipped as partial win; retrieval reopened as 4.12.** Judge-side fixes only: (a) register gate for secular-philosophical seeds + (b) specific-feast match rule with named 4.10A rejects (Mechthild/Catherine on Julian, Memphis Martyrs on Martin) baked into `skills/judge_serendipity/SKILL.md` as worked examples; (c) NOT implemented as a separate rule — register-gating absorbed it; (d) conclusion revised mid-task: retrieval IS the bottleneck after all. `DEFAULT_JUDGE_TIMEOUT` in `scripts/replay_4_7_review.py` bumped 120→180s; `surface.py` chat-path `JUDGE_TIMEOUT=30` untouched. Replay vs 4.10B baseline: `spillover 4/5 → 5/5`, `prose_flips 23 → 17`, `prose_liturgy_spillovers 8 → 0`, `lit_neg 5/5 hold`, `lit_pos 0/10 unchanged`. Wrong-feast pairs (Mechthild/Catherine on Julian, Memphis Martyrs on Martin) now correctly rejected. No judge timeouts. 33.7-minute wall clock. Ship gate `lit_pos ≥ 7/10` NOT met; shipping as partial because every other axis is strict improvement and safety fixes shouldn't wait. **Diagnostic finding:** in the new run, 9 of 10 `lit_pos_*` cases had 0 liturgical candidates reach the judge (`liturgical_hit_count=0`); only `lit_pos_08` surfaced 1 liturgical candidate (wrong feast, correctly rejected by new rubric). Retrieval top-k not surfacing liturgical units for theological seeds — see 4.12. **Hypothesis (unverified):** pre-4.11 baseline file (`build/4_11_pre_replay_baseline.json`, copied from `build/4_7_replay_results.json` before the 4.11 run) may have been generated while 4.10A overfetch was still active — pool sizes in baseline are ~50 vs ~10 in the new run. If true, the "0/10 baseline" was comparing 4.10A-retrieval against 4.11-retrieval; recall regression from overfetch removal was masked by wrong-feast accepts in baseline that 4.11 correctly rejects. Worth verifying via `git log` on `build/4_7_replay_results.json` when scoping 4.12.
- [~] 4.12 — **Liturgical retrieval expansion — REFUTED, edit reverted.** Implemented: `_LITURGICAL_OVERFETCH_MULTIPLIER=2` + supplementary `search(content_type="liturgical_unit", limit=limit*2)` merged by `document_id` into `run_surface()` single-call path. Ruff/mypy/pytest clean. Full replay (~33 min) FAILED 3 of 5 gates: `lit_pos 0/10` (unchanged — 2 of 10 cases newly surfaced liturgical candidates but both wrong feasts: `lit_pos_02` Jonathan Myrick Daniels collect, `lit_pos_03` Pachomius collect — judge correctly rejected per 4.11 rubric; 8 of 10 cases still had 0 liturgical candidates); `prose_flips 17→28` regression (gate ≤17); `prose_liturgy_spillovers 0→5` regression (gate ≤0); `lit_neg 5/5` and `spillover 5/5` held. **Root cause (diagnosed via `/tmp/diag_4_12_liturgical.py`, not committed):** `search()` in `commonplace_server/search.py` applies `content_type` as a POST-KNN filter. `_KNN_OVERFETCH_MULTIPLIER=5` means `search(content_type="liturgical_unit", limit=20)` pulls top-100 nearest chunks across ALL content types then filters — for 8/10 lit_pos cases, zero liturgical units land in the global top-200 (even with 4.12's `limit*2` bump to 200). Corpus census confirms all 8 expected feasts are indexed and embedded (Julian 6588/6589, Martin 6972/6973, Benedict feast 221, Marian 5496/5527, Compline 5741, Ash Wed 5555/5631, Psalm 23 5876, Morning Prayer 5815/5846; total 1393 liturgical units in DB). Bottleneck is not corpus and not supplementary fetch size — it's that filter-after-KNN cannot rescue candidates that aren't already in the global top-N; books crowd out LFF/BCP collects in nomic-embed-text space. Edit reverted; `build/4_7_replay_results.json` restored to HEAD snapshot; 4.12 replay results preserved at `build/4_12_replay_results.json` (untracked) as diagnostic reference. Pivot → 4.13.
- [~] 4.13 — **Filter-aware KNN shipped as architectural foundation; caller wire-up deferred to 4.14.** Refactored `commonplace_server/search.py`: when any document-level filter (`content_type`, `source`, date, liturgical meta) is active, the KNN runs against the restricted chunk set via a vec0 rowid subquery (`WHERE embedding MATCH ? AND chunk_id IN (<filter-predicate subquery>)`) instead of pulling top-(`limit*_KNN_OVERFETCH_MULTIPLIER`) across ALL chunks and post-filtering. No-filter path preserved unchanged. Extracted `_build_filter_predicates()` helper reused by the rowid subquery. Added regression test (`test_filter_aware_knn_rescues_matches_beyond_global_topk`) pinning the 4.12 failure mode: 20 book chunks dominate the global KNN while 2 far liturgical chunks rank 21+; `search(content_type="liturgical_unit", limit=3)` now returns both liturgical hits. Pre-flight spike confirmed vec0 honors compound `MATCH ? AND chunk_id IN (<subquery>)` with `ORDER BY distance LIMIT` — filter applied (not silently ignored; all 20 returned rows verified to match), 67 ms vs 32 ms baseline. Gates: ruff + mypy clean on search.py, 1698/1698 `-m "not live"`. **Replay verdict (full 33-min harness, search.py alone):** `lit_pos 0/10` UNCHANGED — **not because retrieval failed but because the replay path never invokes the fix**: `run_surface()` in ambient mode issues `search(conn, query_blob, limit=limit)` with no `content_type` kwarg, so `has_filters=False` and the filter-aware path is bypassed. Direct probe confirms `search(conn, embed("Julian of Norwich"), content_type="liturgical_unit", limit=10)` now surfaces documents 6588/6589 (expected Julian collects) as top hits. `lit_neg 5/5`, `spillover 5/5` held; `prose_flips 17→16` within noise; `prose_liturgy_spillovers 0` held. **Second attempt in-session — supplementary filter-aware call added to `run_surface()` ambient path (`search(content_type="liturgical_unit", limit=limit)` merged with base by `document_id` on min-score) — regressed:** `prose_flips 14→22` and `prose_liturgy_spillovers 0→2` (gate violations). seed_03 (attention) accepted "Collect for Lydia of Thyatira" 6611:6; seed_10 (work) accepted "Collect for Ignatius of Loyola" 6750:5. Judge register gate did not fire because the liturgical candidates are semantically coherent (Lydia ≈ attentive listening, Ignatius ≈ vocational theology). Reverted surface.py to baseline. **Ship decision:** partial closure on the architectural fix; `commonplace_server/search.py` + `tests/test_search.py` only. **Follow-up 4.14:** (a) investigate apparent replay-harness bug — during the second-attempt replay, 4 lit_pos seeds had the CORRECT expected liturgical docs surface (lit_pos_03 Julian collect, lit_pos_08 Compline collects, lit_pos_09 Ash Wednesday Optional Block) but `matched_expectations=[]` in every case; likely name/kind pair-matcher normalization issue; (b) decide policy on thematic-hit spillovers (true-positive semantic resonance vs gate violation) before re-enabling supplementary liturgical retrieval in `run_surface()`. **Deferred (out of 4.13 scope, unchanged from 4.13 open):** metadata-assisted retrieval — pre-hydrate doc_ids via `feast.primary_name` / `documents.title` for seeds containing named feast/saint tokens.
...
- (2026-04-20) — **Phase 7 task 4.11 shipped as partial win; 4.12 opened.** Judge-side fixes only: (a) register gate for secular-philosophical seeds + (b) specific-feast match rule with named 4.10A rejects (Mechthild/Catherine on Julian, Memphis Martyrs on Martin) baked into `skills/judge_serendipity/SKILL.md` as worked examples. (c) Not a separate rule — register-gating absorbed it. (d) **Conclusion revised mid-task: retrieval IS the bottleneck after all; opening 4.12.** `DEFAULT_JUDGE_TIMEOUT` in `scripts/replay_4_7_review.py` bumped 120→180s; `surface.py` chat-path `JUDGE_TIMEOUT=30` untouched. Replay vs 4.10B baseline: `spillover 4/5 → 5/5`, `prose_flips 23 → 17`, `prose_liturgy_spillovers 8 → 0`, `lit_neg 5/5 hold`, `lit_pos 0/10 unchanged`. Wrong-feast pairs (Mechthild/Catherine on Julian, Memphis Martyrs on Martin) now correctly rejected. No judge timeouts. 33.7-minute wall clock. Ship gate `lit_pos ≥ 7/10` not met; shipping as partial win because every other axis is strict improvement and safety fixes shouldn't wait. Diagnostic finding: 9 of 10 `lit_pos_*` cases had 0 liturgical candidates reach the judge (`liturgical_hit_count=0`); only `lit_pos_08` surfaced 1 liturgical candidate (wrong feast, correctly rejected by new rubric). Retrieval top-k not surfacing liturgical units for theological seeds. Hypothesis (unverified): pre-4.11 baseline file (`build/4_11_pre_replay_baseline.json`, copied from `build/4_7_replay_results.json` before the 4.11 run) may have been generated while 4.10A overfetch was still active — pool sizes in baseline are ~50 vs ~10 in the new run. If true, the "0/10 baseline" was comparing 4.10A-retrieval against 4.11-retrieval; recall regression from overfetch removal was masked by wrong-feast accepts in baseline that 4.11 correctly rejects. Worth verifying via `git log` on `build/4_7_replay_results.json` when scoping 4.12. **4.12 opened:** re-attempt liturgical retrieval expansion narrower than 4.10A's reverted `top_k*5` overfetch (probably 2× for the liturgical index only), gated on seed register where cheap. 4.11's tightened rubric now correctly rejects the wrong-feast leakage that came with 4.10A expansion, so the judge-side failure mode is closed. First step before coding: verify the baseline-file hypothesis via `git log`.
