...
**Last update:** 2026-04-19 (Phase 7 Wave 4B + 4.7 reviewed — prose side clears Moderate bar; liturgical side blocked on retrieval-layer bug; opened 4.8 as blocker for Phase 4 close)
**Status:** in_progress — Phase 1 complete; LFF feast-backfill complete; Wave 4A + 4B all shipped; 4.7 partial (prose ratified; liturgical blocked on 4.8). 4.8 is the last Phase 4 gate: backport `embed_text_override` into `liturgy_bcp.py`, reingest BCP, live-ingest LFF, rerun replay.
...
- [ ] 4.8 — **Backport `embed_text_override` into `liturgy_bcp.py` + reingest BCP + live-ingest LFF + rerun replay.** Mirror `liturgy_lff.py:309` pattern; compose per-category embed strings (collect/daily-office/psalter/proper-liturgy/prayer-thanksgiving) with feast/rite/seasonal context. Purge existing BCP docs+chunks+embeddings+meta so fresh vectors generate (idempotent handler short-circuits otherwise). Enqueue `ingest_liturgy_lff` — live `library.db` has 0 rows at `source='lff_2024'` (lit_pos_04/05/06 block on this). Rerun `scripts/replay_4_7_review.py`; expect positives to surface.
